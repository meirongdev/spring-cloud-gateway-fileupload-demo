...
- 连接池 per-JVM：`HttpClient` 实例在每个网关 pod 里各一份；
- kube-proxy 按 **TCP 连接**做负载均衡：一条 h2 连接固定落在某一个
  后端 pod 上（这也是验证输出里 `servedBy` 恒定的原因）；
- 流量恒定的前提下：1 个 pod = 单池全量流量保温，空闲 >20s 几乎不
  发生；3 个 pod = 每池 1/3 流量，空闲窗口频繁 → 冷连接上传概率 ≈
  「该 pod 距上一个请求超过 20s 的概率」→ **偶发**，且换 pod 数直接改变
  发生率 —— 与观察完全吻合。
